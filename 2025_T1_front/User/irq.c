...
/***************************奇妙变量**********************************/
extern unsigned int motor_button;
extern int delta_duty,duty_left,duty_right;
extern sint16 ECPULSE1;
extern sint16 ECPULSE2;
extern int roundabout_flag;
extern unsigned char Image_Go[60][80];
//extern float wucha_zuo, piancha_all;
/***************************定时器中断回调函数**********************************/
...
void STM0_CH1_IRQHandler(void)
{
    ...
    */
    if (Camera_Flag == 2)
    {
        ...
        Smaller_Use_Image2();
//        Bin_Image_Filter2();
//        image_draw_rectan(Image_Go);
        saoxian2();
//        image_process();
        Camera_Flag = 0;
    }
    else LED_Ctrl(LED0, RVS);
}
...
void STM1_CH0_IRQHandler(void)
{
    ...
    */
//    char txt[32];
//    Reverse_And_Enlarge_Image();
//    Draw_Center_Line_Zoom();
    Display_Show(0, 0, LCDH, LCDW, (unsigned char *)Pixle);
//    sprintf(txt, "shape:%d", roundabout_flag);             // 将变量填充到字符串的对应位置，并将字符串存放到txt[]中
//    LCD_ShowStr_24(0, 3, txt, U16_RED, U16_BLUE); // 显示24*12字符串
//    sprintf(txt, "wucha_zuo:%.3f", wucha_zuo);             // 将变量填充到字符串的对应位置，并将字符串存放到txt[]中
//    LCD_ShowStr_24(0, 4, txt, U16_RED, U16_BLUE); // 显示24*12字符串
//    sprintf(txt, "speed_zuo: %5d;", duty_left);
//    Display_showString(0, 8, txt, U16_WHITE, U16_BLACK, 16); // 字符串显示
//    sprintf(txt, "speed_you: %5d;", duty_right);
//    Display_showString(0, 9, txt, U16_WHITE, U16_BLACK, 16); // 字符串显示
