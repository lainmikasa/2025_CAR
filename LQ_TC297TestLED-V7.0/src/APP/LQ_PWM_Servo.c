...
void ServoInit(void)
{
    ...
    ATOM_PWM_InitConfig(ATOMSERVO3, Servo_Center_Mid, Servo_Frequency); // 舵机频率为100HZ，初始值为1.5ms中值
    ATOM_PWM_InitConfig(ATOMSERVO4, Servo_Center_Mid, Servo_Frequency); // 舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
...
//        duty = Servo_Right_Min;
    ...
    case Servo2:
        ...
        break;
    case Servo3:
        ATOM_PWM_InitConfig(ATOMSERVO3, duty, Servo_Frequency); // 舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
        break;
    case Servo4:
        ATOM_PWM_InitConfig(ATOMSERVO4, duty, Servo_Frequency); // 舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
        break;
    case ServoALL:
        ATOM_PWM_InitConfig(ATOMSERVO1, duty, Servo_Frequency); // 舵机频率为50HZ，初始值为1.5ms中值
        ATOM_PWM_InitConfig(ATOMSERVO2, duty, Servo_Frequency); // 舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
        ATOM_PWM_InitConfig(ATOMSERVO3, duty, Servo_Frequency); // 舵机频率为50HZ，初始值为1.5ms中值
        ATOM_PWM_InitConfig(ATOMSERVO4, duty, Servo_Frequency); // 舵机理论范围为：0.5ms--2.5ms，大多舵机实际比这个范围小
        break;
    }
}
...
void TestServo(void)
{
    ...
    ServoCtrl(Servo1, 980*6);
    ServoCtrl(Servo2, 440*6);
    ServoCtrl(Servo3, 980*6);
    ServoCtrl(Servo4, 440*6);
    while (1)
    {
        if (!KEY_Read(KEY0))
        {
        ...
        if (!KEY_Read(KEY2))
        {
            Delay_Ms(3000);
            //duty += 10;
            ServoCtrl(Servo1, 440*6);  // 980  2640
            ServoCtrl(Servo2, 980*6);  // 440  8820
            ServoCtrl(Servo3, 440*6);  // 980
            ServoCtrl(Servo4, 980*6);  // 440
            Delay_Ms(duty);
            ServoCtrl(Servo1, 980*6);
            ServoCtrl(Servo2, 440*6);
            ServoCtrl(Servo3, 980*6);
...
//        ServoCtrl(Servo1, 980);
//        ServoCtrl(Servo2, 440);
//        ServoCtrl(Servo3, 980);
//        ServoCtrl(Servo4, 440);
