...
{
   PIN_InitConfig(P00_5, IfxPort_Mode_inputPullUp, 1);
...
uint8_t Get_Hall_State(void)
{
  ...
  if(PIN_Read(P20_3)) //F11/* 读取霍尔传感器 U 的状态 */
  {
    hall_state |= 0x01U << 0;
  }
  if(PIN_Read(P20_0)) //F12/* 读取霍尔传感器 V 的状态 */
  {
    hall_state |= 0x01U << 1;
  }
  if(PIN_Read(P00_5)) //F13 /* 读取霍尔传感器 W 的状态 */
  {
    hall_state |= 0x01U << 2;
  }
  return hall_state;
